<!--
.. title: 3D Sequential Optical System Layouts
.. slug: 3d-sequential-optical-system-layouts
.. date: 2025-06-05 11:26:25 UTC+02:00
.. tags: ray tracing, optics
.. category: programming
.. link: 
.. description: I describe the geometry involved for laying out sequential optical systems in 3D.
.. type: text
.. has_math: true
-->
...
A rotation from the global frame into the cursor frame can be achieved by taking the \\( \hat{ r } \\), \\( \hat{ u } \\), and \\( \hat{ f } \\) unit vectors that define the cursor frame in the global coordinate system and making them the rows of a \\( 3 \times 3 \\) rotation matrix. At the second mirror, this matrix is:
...
If this is not clear, consider that the rows of a rotation matrix represent the basis vectors of the coordinate system after a passive rotation, but expressed in the original (global) frame's coordinate system. (See below for more on passive rotations.) When you multiply a global-frame vector by this matrix, each row makes a dot product with that vector, projecting it onto the corresponding cursor axis. From the diagram above, \\( \hat{ u } \left( s_2 \right) = ( 0, 1 / 2, \sqrt{ 3 } / 2)^{ \mathrm{ T } } \\) and \\( \hat{ f } \left( s_2 \right) = ( 0, \sqrt{ 3 } / 2, - 1 / 2)^{ \mathrm{ T
  }} \\), which are the second and third rows of the matrix.
